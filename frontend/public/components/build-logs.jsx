...
const PipelineLogMessage = ({ build }) => {
  ...
  const message = logURL
    ? t('build-logs~Pipeline build logs are available through Jenkins (linked below)')
    : t(
        'build-logs~A link to the Jenkins pipeline build logs will appear below when the build starts',
      );
  ...
  return <MsgBox title={t('build-logs~See Jenkins log')} detail={detail} />;
};
