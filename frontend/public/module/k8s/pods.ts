  ...
  _.find(
    {
      Always: {
        ...
        label: i18next.t('pods~Always restart'),
        // Description in the UI.
        description: i18next.t(
          'pods~If the container restarts for any reason, restart it. Useful for stateless services that may fail from time to time.',
        ),
      ...
      OnFailure: {
        id: 'OnFailure',
        label: i18next.t('pods~Restart on failure'),
        description: i18next.t(
          'pods~If the container exits with a non-zero status code, restart it.',
        ),
      },
      Never: {
        id: 'Never',
        label: i18next.t('pods~Never restart'),
        description: i18next.t(
          'pods~Never restart the container. Useful for containers that exit when they have completed a specific job, like a data import daemon.',
        ),
      },
    },
...
export const VolumeSource = {
  emptyDir: {
    id: 'emptyDir',
    label: i18next.t('pods~Container volume'),
    description: i18next.t("pods~Temporary directory that shares a pod's lifetime."),
  },
  hostPath: {
    id: 'hostPath',
    label: i18next.t('pods~Host directory'),
    description: i18next.t(
      'pods~Pre-existing host file or directory, generally for privileged system daemons or other agents tied to the host.',
    ),
  },
  gitRepo: {
    id: 'gitRepo',
    label: i18next.t('pods~Git repo'),
    description: i18next.t('pods~Git repository at a particular revision.'),
  },
  nfs: {
    id: 'nfs',
    label: i18next.t('pods~NFS'),
    description: i18next.t('pods~NFS volume that will be mounted in the host machine.'),
  },
  secret: {
    id: 'secret',
    label: i18next.t('pods~Secret'),
    description: i18next.t('pods~Secret to populate volume.'),
  },
  gcePersistentDisk: {
    id: 'gcePersistentDisk',
    label: i18next.t('pods~GCE Persistent Disk'),
    description: i18next.t('pods~GCE disk resource attached to the host machine on demand.'),
  },
  awsElasticBlockStore: {
    id: 'awsElasticBlockStore',
    label: i18next.t('pods~AWS Elastic Block Store'),
    description: i18next.t('pods~AWS disk resource attached to the host machine on demand.'),
  },
  glusterfs: {
    id: 'glusterfs',
    label: i18next.t('pods~Gluster FS'),
    description: i18next.t('pods~GlusterFS volume that will be mounted on the host machine.'),
  },
  iscsi: {
    id: 'iscsi',
    label: i18next.t('pods~iSCSI'),
    description: i18next.t('pods~iSCSI disk attached to host machine on demand'),
  },
  configMap: {
    id: 'configMap',
    label: i18next.t('pods~ConfigMap'),
    description: i18next.t('pods~ConfigMap to be consumed in volume.'),
  },
  projected: {
    id: 'projected',
    label: i18next.t('pods~Projected'),
    description: i18next.t(
      'pods~A projected volume maps several existing volume sources into the same directory.',
    ),
  },
};
